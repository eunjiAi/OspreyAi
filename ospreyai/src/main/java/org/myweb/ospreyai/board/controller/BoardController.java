...
import org.myweb.ospreyai.board.model.service.BoardService;
...
public class BoardController {
	...
	public String boardTop3Method(HttpServletResponse response) throws UnsupportedEncodingException {
		...
		// log.info("btop3.do : " + list.toString());
	 ...
	 * public String replyInsertMethod(Board reply, Model
	 * model, @RequestParam("bnum") int bnum,
	 * 
	 ...
	 * public String replyUpdateMethod(Board reply, @RequestParam("page") int
	 ...
	 * public String originUpdateMethod(Board board, Model model, HttpServletRequest
	 * request,
	 * 
	 * @RequestParam(name = "page", required = false) String page,
	 * 
	 ...
	 * logger.info("borginupdate.do : " + board); // 전송온 값 확인
	 * 
	 * int currentPage = 1;
	 * if (page != null) {
	 * currentPage = Integer.parseInt(page);
	 ...
	 * // => 이전 파일과 파일정보 삭제함
	 * if (board.getBoardOriginalFilename() != null && (delFlag != null &&
	 * delFlag.equals("yes"))) {
	 * // 저장 폴더에서 이전 파일은 삭제함
	 * new File(savePath + "\\" + board.getBoardRenameFilename()).delete();
	 * // board 안의 파일 정보도 삭제함
	 ...
	 * model.addAttribute("page", currentPage);
	 * 
	 * return "redirect:bdetail.do?bnum="+ board.getBoardNum() + "&page=" +
	 * currentPage;
	 * } else {
	 ...
	 * 
	 * // 마이바티스 매퍼에서 사용되는 메소드는 Object 1개만 전달할 수 있음
	 * // paging.startRow, paging.endRow + search.begin, search.end 같이 전달해야 하므로 =>
	 * 하나의
	 * // 객체로 만들어야 함
	 * search.setStartRow(paging.getStartRow());
	 * search.setEndRow(paging.getEndRow());
