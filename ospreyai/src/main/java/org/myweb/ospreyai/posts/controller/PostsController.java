...
public class PostsController {
	...
	public ResponseEntity<Map> noticeSearchTitleMethod(
		...
		int listCount = postsService.selectSearchTitleCount(keyword);
		// 페이지 관련 항목 계산 처리
		...
		ArrayList<Posts> list = postsService.selectSearchTitle(keyword, pageable);
...
//		// page : 출력할 페이지, limit : 한 페이지에 출력할 목록 갯수
//		// 검색결과가 적용된 총 목록갯수 조회해서 총 페이지 수 계산함
//		int listCount = postsService.selectSearchContentCount(keyword);
//		// 페이지 관련 항목 계산 처리
...
//
//		// 서비스롤 목록 조회 요청하고 결과 받기
//		ArrayList<Posts> list = postsService.selectSearchContent(keyword, pageable);
//		Map<String, Object> map = new HashMap<String, Object>();
//
//		if (list != null && list.size() > 0) {
//			map.put("list", list);
