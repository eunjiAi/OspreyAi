...
public class NoticeController {
	...
	public String noticeInsertMethod(Notice notice, Model model,
		...
		// importance가 null일 경우 "N"으로 설정
		if (notice.getImportance() == null) {
			notice.setImportance("N");
		}

		// impEndDate를 현재 시간으로 설정
		if (notice.getImpEndDate() == null || notice.getImpEndDate().isEmpty()) {
			notice.setImpEndDate(java.sql.Date.valueOf(java.time.LocalDate.now()).toString());
		}

		// 로그 출력 (변경된 notice 객체를 로그에 출력)
	...
	public String noticeUpdateMethod(Notice notice, Model model, HttpServletRequest request,
		...
		notice.setNoticeDate(new java.sql.Date(System.currentTimeMillis()).toString());

		if (noticeService.updateNotice(notice) > 0) { // 공지글 수정 성공시
			model.addAttribute("no", notice.getNoticeNo());
			model.addAttribute("page", currentPage);
			return "redirect:ndetail.do?no=" + notice.getNoticeNo() + "&page=" + currentPage;
		} else {
